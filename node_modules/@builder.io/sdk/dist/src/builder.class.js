    ...
    try {
        ...
        document.cookie =
            name +
                ...
                expiresString +
                '; path=/' +
                ("; domain=" + get_top_level_domain_1.getTopLevelDomain(location.hostname)) +
...
var Builder = /** @class */ (function () {
    ...
    Builder.prototype.processEventsQueue = function () {
        ...
        var host = this.host;
        fetch_function_1.fetch(host + "/api/v1/track", {
            method: 'POST',
    ...
    Object.defineProperty(Builder.prototype, "browserTrackingDisabled", {
        get: function () {
            return Boolean(Builder.isBrowser && (navigator.doNotTrack === '1' || window.builderNoTrack));
        },
        enumerable: true,
        configurable: true
    });
    ...
    Builder.prototype.track = function (eventName, properties, context) {
        ...
        var eventData = JSON.parse(JSON.stringify({
            type: eventName,
            data: __assign(__assign({}, omit_function_1.omit(properties, 'meta')), { metadata: __assign(__assign({ sdkVersion: Builder.VERSION, url: location.href }, properties.meta), properties.metadata), ownerId: this.apiKey, userAttributes: this.getUserAttributes(), sessionId: this.sessionId, visitorId: this.visitorId }),
        }));
    ...
    Builder.prototype.requestUrl = function (url) {
        if (Builder.isBrowser) {
            ...
            return fetch_function_1.fetch(url, this.authToken
                ? {
                    headers: {
        ...
        return new Promise(function (resolve, reject) {
            var module = url.indexOf('http:') === 0 ? server_only_require_function_1.default('http') : server_only_require_function_1.default('https');
            module
    ...
    Builder.prototype.flushGetContentQueue = function (usePastQueue, useQueue) {
        ...
        var promise = this.requestUrl(host + "/api/v1/" + (format === 'solid' || format === 'react' ? 'codegen' : 'query') + "/" + this.apiKey + "/" + keyNames + (queryParams && hasParams ? "?" + queryStr : '')).then(function (result) {
            for (var _i = 0, queue_3 = queue; _i < queue_3.length; _i++) {
                ...
                if (data) {
                    var testModifiedResults = Builder.isStatic
                        ? sorted
    ...
    Builder.prototype.getAll = function (modelName, options) {
        ...
        return instance
            .getContent(modelName, __assign(__assign({ limit: 30 }, options), { key: options.key ||
                ...
                (Builder.isBrowser &&
                    modelName + ":" + hash_sum_1.default(omit_function_1.omit(options, 'initialContent', 'req', 'res'))) ||
                undefined }))
            .promise();
    };
